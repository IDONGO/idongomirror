        ...
        for index, download in enumerate(list(download_dict.values())[START:], start=1):
            ...
            if download.status() not in [
                ...
                if download.status() == MirrorStatus.STATUS_CLONING:
                    msg += f"\n<b>♻️</b> {get_readable_file_size(download.processed_bytes())} dari {download.size()}"
                elif download.status() == MirrorStatus.STATUS_UPLOADING:
                    msg += f"\n<b>📦</b> {get_readable_file_size(download.processed_bytes())} dari {download.size()}"
                else:
                    msg += f"\n<b>📦</b> {get_readable_file_size(download.processed_bytes())} dari {download.size()}"
                msg += f"\n<b>⚡️</b> {download.speed()} | <b>⏱</b> {download.eta()}"
                try:
                ...
                try:
                    ...
                    pass
                try:
                    msg += f"\n<b>👻 </b><b>{download.message.from_user.first_name}</b>\n<b>⚠️ </b><code>/warn {download.message.from_user.id}</code>"
                except:
                    pass
